...
*Calibration parameters
 ...
 PRODSHR(A,RD)
*fh
 yield0(A)               crop yield (tonne per hectare)
*fh energy - utax
...
*CA specify base level of intermediate input use
 ica0(C,A,RD)$(QINTA0(A,RD) AND PQ0(C))
                = SUM(ARD$MARD(ARD,A,RD), SAM(C,ARD))/PQ0(C) / QINTA0(A,RD) ;
...
$ontext
 ...
 BUDSHR2(A,H)$(SUM(CP, SAM(CP,H)) + SUM(AP, QHA0(AP,H)*PA0(AP)))
         = QHA0(A,H)*PA0(A)
...
$ontext
*Option 1: LES household expenditure
...
*$ontext
*Option 2: Cobb-Douglas household expenditure
 betam(C,H)   = BUDSHR(C,H);
 betam0(C,H)  = betam(C,H);
 betah(A,H)   = BUDSHR2(A,H);
...
*fh
 yield0(A)$QF0('FLND',A,'NAT')   = QA0(A)/QF0('FLND',A,'NAT');
...
*utax code
 ...
 WFDIST0(F,AAG,'NAT')$QF0(F,AAG,'NAT') = SUM(RDNT, QF0(F,AAG,RDNT)*WF0(F)*WFDIST0(F,AAG,RDNT))
...
*Activity taxes
 tabar(AAG,'NAT')   = SUM(RDNT, tabar(AAG,RDNT)*PAR0(AAG,RDNT)*QAR0(AAG,RDNT))
                     / SUM(RDNT, PAR0(AAG,RDNT)*QAR0(AAG,RDNT));
...
*Price block
 ...
 PMDEF(C,RW)$CMRW(C,RW)..        PM(C,RW) =E= PWM(C,RW)*(1 + tm(C,RW))*EXR + SUM(CT, PQ(CT)*icm(CT,C));

 PEDEF(C,RW)$CERW(C,RW)..        PE(C,RW) =E= (PWE(C,RW)*(1 - te(C,RW)))*EXR - SUM(CT, PQ(CT)*ice(CT,C));
...
*fh energy - utax
 ...
 PQDEF(C)$((CD(C) OR CM(C)))..   PQ(C)*(1 - TQ(C))*QQ(C) =E= PDD(C)*QD(C) + SUM(RW, PM(C,RW)*QM(C,RW));
...
*PERFECT SUBSTITUTION ACTIVITY AGGREGATION FUNCTION
...
*CES REGIONAL ACTIVITY AGGREGATION FUNCTION
 QACES(A)$(ACES2(A) AND QA0(A))..  QA(A) =E=
   alphaa2(A)*(SUM(RD$deltaa2(A,RD), deltaa2(A,RD)*QAR(A,RD)**(-rhoa2(A))))**(-1/rhoa2(A));

* QACESFOC(A,RD)$(ACES2(A) AND QA0(A) AND deltaa2(A,RD)).. PAR(A,RD) =E=
 QACESFOC(A,RD)$(ACES2(A) AND QA0(A) AND QAR0(A,RD)).. PAR(A,RD) =E=
   PA(A)*deltaa2(A,RD)*(SUM(RDP$deltaa2(A,RDP), deltaa2(A,RDP)*QAR(A,RDP)**(-rhoa2(A))))**(-1)*QA(A)*QAR(A,RD)**(-rhoa2(A)-1);
...
*JTSACT Need to add a nested CES for purely imported products with multiple import regions (similarly for 100% exports)
...
*fh energy - utax
 ...
 CURACCBAL..
   SUM((CM,RW), PWM(CM,RW)*QM(CM,RW)) + SUM(F, trnsfr('ROW',F)) + SUM(F, (1-tf(f))*rf(F)*YF(F))/EXR
         =E= SUM((CE,RW), PWE(CE,RW)*QE(CE,RW)) + SUM(INSD, trnsfr(INSD,'ROW')) + (FSAV+FBOR - irate*debt);
...
** TVADEF(A,RD)..    TVA(A,RD)*TAXADJ =E= tvabar(A,RD) * (1+TVAADJ*tva01(A,RD)) + TVAPS*tva01(A,RD);
 ...
 SAVINVBAL..
   SUM(INSDNG, MPS(INSDNG) * (1 - TINS(INSDNG)) * YI(INSDNG)) + GSAV + (FSAV+FBOR - irate*debt)*EXR =E=
